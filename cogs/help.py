...
class help(commands.Cog):
    ...
    async def help(self,ctx,view = 'General'):
        ...
        if view == 'Verify':
            ...
            embed = discord.Embed(title = 'Bazaar Command', color = discord.Color.blue(), description = 'Usage: ``!bazaar <product>``')
            embed.add_field(name = 'Info', value = 'Displays bazaar product info',inline=False)
            embed.add_field(name = 'Aliases', value = '```!bz```',inline=False)
            embed.add_field(name = 'Example', value = 'Displays info for lapis\n```!bz lapis```',inline=False)
            await channel.send(embed=embed)     
        else:
